...
func Token2022MintTo(t *testing.T, ctx context.Context, rpcClient *rpc.Client, wsClient *ws.Client, mint, payer, partner *solana.Wallet) {
	ctx1, cancel1 := context.WithTimeout(ctx, time.Second*30)
	defer cancel1()
	...
	// Token2022 ProgramID
	token2022ProgramID := token2022.ProgramID
	...
	if err != nil {
		t.Fatal("GetMinimumBalanceForRentExemption fail", err)
	}
	...
	// 初始化 mint
	initMintIx := token2022.NewInitializeMint2Instruction(
		6, // decimals
		payer.PublicKey(),
		solana.PublicKey{}, // freeze authority
		mint.PublicKey(),
	).Build()

	// payer ATA
	ata, _, err := solana.FindAssociatedTokenAddressWithProgram(
		payer.PublicKey(),
		mint.PublicKey(),
		token2022ProgramID,
	)
	...
	// 创建 payer ATA
	createAtaIx := associatedtokenaccount.NewCreateInstructionWithTokenProgram(
		payer.PublicKey(), // payer
		payer.PublicKey(), // wallet
		mint.PublicKey(),  // mint
		token2022ProgramID,
	).Build()

	// mint to payer
	mintToIx := token2022.NewMintToInstruction(
		mintAmount,
		mint.PublicKey(),
		ata,
		payer.PublicKey(),
	...
	// partner ATA
	partnerAta, _, err := solana.FindAssociatedTokenAddressWithProgram(
		partner.PublicKey(),
		mint.PublicKey(),
		token2022ProgramID,
	)
	...
	// 创建 partner ATA
	createPartnerAtaIx := associatedtokenaccount.NewCreateInstructionWithTokenProgram(
		payer.PublicKey(),
		partner.PublicKey(),
		mint.PublicKey(),
		token2022ProgramID,
	...
	// mint to partner
	mintToPartnerIx := token2022.NewMintToInstruction(
		100_000_000*1e6,
		mint.PublicKey(),
		partnerAta,
		payer.PublicKey(),
	...
	fmt.Println("Token2022MintTo success", sig.String())
}
